...
const matchPath = (path: string) => {
  if(path.match("/organizations*")) {
    ...
    return "apps";
  } else if(path === "/" || path.match("/launch*")) {
    return "launch";
  }
}
  ...
  <Layout style={{ maxHeight: '100vh' }}>
    <Sider collapsible>
      ...
      <Menu theme="dark" mode="inline" activeKey={matchPath(route.location.pathname)} selectedKeys={[matchPath(route.location.pathname)]}>
        <Menu.Item key="launch"><Link to='/'><Icon type={'code-o'} style={{ marginRight: 8 }} />Launch</Link></Menu.Item>
        <Menu.Item key="apps"><Link to='/apps'><Icon type={'appstore'} style={{ marginRight: 8 }} />Apps</Link></Menu.Item>
        <Menu.Item key="orgs"><Link to='/organizations'><Icon type={'global'} style={{ marginRight: 8 }} />Organizations</Link></Menu.Item>
      </Menu>
    </Sider>
    <Layout>
      <Content>
        <Route exact path='/organizations' component={Organizations as any} />
        <Route exact path='/' component={AppLaunch as any} />
